...
for (const file of files) {
  ...
  const sourcesCaptures = await sources.reduce(async (sourcesCaptures, source) => {
  ...
  for (const tribune of tribunes) {
    ...
    groupAggregation.authors = [...new Set(groupAggregation.authors.concat(author))].sort();
    ...
    writerAggregation.groups = [...new Set(writerAggregation.groups.concat(group))];
    writerAggregation.groupsIds = [...new Set(writerAggregation.groupsIds.concat(groupDetails.id))];
    writerAggregation.mandates = [...new Set(writerAggregation.mandates.concat(title))];
    ...
    const bold = {
      ...
      // use the AST
      count: Math.floor([...(content.matchAll(/\*\*/mg))]?.length / 2) || 0,
    };
    ...
    words.forEach(word => {
      ...
      writerAggregation.words[lemma] = (writerAggregation.words[lemma] || 0) + 1;
      groupAggregation.words[lemma] = (groupAggregation.words[lemma] || 0) + 1;
    });
...
${content}
`;
    await fs.promises.writeFile(pathJoin("contents", "tribunes", `${id}.md`), markdown);
  }
}
    ...
    words: Object.keys(writtersAggregations[key].words)
      ...
      .sort((wordA, wordB) => (
        writtersAggregations[key].words[wordA] <
          writtersAggregations[key].words[wordB] ? 1 :
          writtersAggregations[key].words[wordA] >
            writtersAggregations[key].words[wordB] ? -1 :
            0
      ))
      .slice(0, 25)
      .map(word => ({ word, count: writtersAggregations[key].words[word] }))
  };
    ...
    words: Object.keys(groupsAggregations[key].words)
      ...
      .sort((wordA, wordB) => (
        groupsAggregations[key].words[wordA] <
          groupsAggregations[key].words[wordB] ? 1 :
          groupsAggregations[key].words[wordA] >
            groupsAggregations[key].words[wordB] ? -1 :
            0
      ))
